...
public class ProductsHelper {
    ...
    public static final String PRODUCTS_LIST = CENTER_COLUMN + "//ul[contains(@class, 'product_list')]";
    ...
    public static WebElement getProduct(WebDriver dvr, int index) {
        return AppHelper.findElement(dvr, PRODUCTS_LIST + "/li[" + index + "]");
    }
    ...
    public static String getProductName(WebDriver dvr, WebElement product) {
        try {
            return product.findElement(By.xpath("./*//child::a[@class='product-name']")).getText();
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public static double getProductPrice(WebDriver dvr, WebElement product) {
        int index = 2;
        if (getViewType(dvr).equalsIgnoreCase("list"))
            index = 3;
        return Double.parseDouble(
                product.findElement(By.xpath(".//div[" + index + "]//child::span[contains(@class, 'price')]")).getText()
                        .replace("$", ""));
    }
